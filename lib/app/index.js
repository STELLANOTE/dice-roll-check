      ...
      if (signs.length > 3) {
        ...
        switch (signs.length) {
          case 0: {
            ...
            result.info.rollResult = `${ leftPart.str } => ${ leftPart.sum }`
...
function checkRolledResults (sign, leftPart, rightPart) {
  ...
  const rollResultText = `${ rollResult } => ${ leftPart.sum }${ sign }${ rightPart.sum } => ${ isRollSucceed ? ROLL_SUCCEED_TEXT : ROLL_FAILED_TEXT }`
  const rollResultSimple = `${ rollResult } => ${ isRollSucceed ? ROLL_SUCCEED_TEXT : ROLL_FAILED_TEXT }`
