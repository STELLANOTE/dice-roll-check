      ...
      if (signs.length > 3) {
        ...
        switch (signs.length) {
          ...
          case 1: {
            ...
            const [isRollSucceed, rollResult] = checkRolledResults (signs[0], leftPart, rightPart)
...
function checkRolledResults (sign, leftPart, rightPart) {
  ...
  const rollResult = `${ leftPart.str }${ sign }${ rightPart.str } => ${ leftPart.sum }${ sign }${ rightPart.sum } => ${ isRollSucceed ? ROLL_SUCCEED_TEXT : ROLL_FAILED_TEXT }`
